...
| high 1 + low 1 + 缺失/超时 | `free_walk` | `[0,0,0]` velocity | 模型输出 |
...
- 进入或切换导航语义后，尚未收到对应新参数时三元组使用 `[0,0,0]`。high 1/low 1
  保持 `free_walk`；high 1/low 2 保持 `accurate_arrival`；high 3/low 1
  保持 `arm_walk`。
...
1. 检查 LowState 新鲜度，并读取四个踝关节电机的两个温度通道；任一通道严格
   超过 70°C 时输出限频温度告警。
...
控制进程直接读取 Unitree `rt/lowstate.motor_state[].temperature`。监测对象固定为
左右踝 pitch/roll 共四个关节；DDS 电机下标通过 `motor_joint_names` 和
`motor_indices` 映射，不假设配置数组顺序与模型顺序相同。每个电机读取 SDK 提供的
两个温度通道，任一通道严格超过 70°C 时打印
`[MOTOR_TEMPERATURE_WARNING]`。持续过热最多每 5 秒打印一次；全部恢复后再次过热
会立即打印。该功能只报警，不自动切模或停止 LowCmd。

控制器不输出逐帧 `[INFERENCE]`，防止 50 Hz 终端刷屏。推理前 observation 使用
`policy_input` topic 观测；`accurate_arrival` 的完整推理输出使用 ToTarget 日志。
